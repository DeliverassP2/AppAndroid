...
function editar() {
    ...
    xhttp.open("POST", "http://192.168.1.203/editar.php", true);
...
function absent() {
    ...
    var parametres = document.getElementById("referencia").textContent;
    console.log(parametres);
    xhttp.open("POST", "http://192.168.1.203/absent.php", true);
    xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    xhttp.send("id=" + parametres);
    alert("Entrega numero " + parametres + " marcada com absent!");
    location.reload();
}
...
function upload() {
    var xhttp = new XMLHttpRequest();
    xhttp.open("POST", "http://192.168.1.203/xmlPostgres.php", true);
    xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    xhttp.send();
...
function lecturaxml(xml) {
    ...
    for (var i = 0; i < xmlDoc.getElementsByTagName("pedido").length; i++) {
        ...
        locations.push([i, latitud, longitud, id_enviament, direccio, dataEnv, estat, intents]);
...
var app = {
    ...
    onSuccess: function(position) {
        ...
        for (var i = 0; i < locations.length; i++) {
            if (locations[i][7] == "1" || locations[i][7] == "2") {
                marker = new L.marker([locations[i][1], locations[i][2]], { icon: orangeIcon }).bindPopup("<button id='cameraTakePicture' onclick='cameraTakePicture()'><i class='fas fa-camera'></i>&nbsp;FOTO</button><br><img id='ImatgeCamara' src=''/ ><br><button id='btnEntregar' onclick='editar()'><i class='fas fa-check'></i>&nbsp;ENTREGAT</button><br><br><b>Ref.: </b><span id='referencia'>" + locations[i][3] + "</span><br><b>Direccio: </b>" + locations[i][4] + "<br><b>Entrega: </b>" + locations[i][5] + "<br><b>Estat: </b>" + locations[i][6] + "<br><b>Intents d'entrega: </b>" + locations[i][7]).addTo(map);
            } else if (locations[i][6] == 'No entregat') {
                marker = new L.marker([locations[i][1], locations[i][2]], { icon: redIcon }).bindPopup("<button id='cameraTakePicture' onclick='cameraTakePicture()'><i class='fas fa-camera'></i>&nbsp;FOTO</button><br><img id='ImatgeCamara' src=''/ ><br><button id='btnEntregar' onclick='editar()'><i class='fas fa-check'></i>&nbsp;ENTREGAT</button><br><button id='btnAbsent' onclick='absent()'><i class='fas fa-check'></i>&nbsp;ABSENT</button><br><b>Ref.: </b><span id='referencia'>" + locations[i][3] + "</span><br><b>Direccio: </b>" + locations[i][4] + "<br><b>Entrega: </b>" + locations[i][5] + "<br><b>Estat: </b>" + locations[i][6] + "<br><b>Intents d'entrega: </b>" + locations[i][7]).addTo(map);
            } else {
